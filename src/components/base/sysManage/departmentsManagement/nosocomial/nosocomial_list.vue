...
<template>
<layout-tree>
  <left-tree slot="left" @tree-click="treeClick" @setCurrSltNodeId="setTreeDepId" :treeOptions="treeDefaults" :fromWhereTreeType="fromWhereTree"></left-tree>
      ...
      <el-table
        ...
        >
          <template scope="scope">
            ...
            <el-button size="small" v-if="scope.row.enable==0" class="but-col" @click="forbidden(scope.$index, scope.row)" type="danger">禁用</el-button>
            <el-button size="small" v-if="scope.row.enable==1" class="but-col" @click="startUsing(scope.$index, scope.row)">启用</el-button>
          </template>
        </el-table-column>
        <el-table-column
          prop="name"
