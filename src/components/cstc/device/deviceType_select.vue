          ...
          <el-table align="center" ref="multipleTable" :height="dynamicHt" :context="self" :data="tableData" tooltip-effect="dark" style="width: 100%">
            ...
            <el-table-column type="expand">
              <template scope="props">
                <el-table align="center" :context="self" :data="props.row.deviceList" tooltip-effect="dark"
                          @select="handleSelect"
                          @select-all="handleSelectionChange"
                          @header-click="handleheaderClick(props.row.deviceList)"
                >
                  <el-table-column type="selection" width="55"></el-table-column>
...
//            reserveTimeSetId: this.timeData.reserveTimeSetId,
      ...
      handleheaderClick(column){
        this.selectTableTeml = column;
      },
      // 单选
      handleSelect(row,index){
        this.selectTableTeml = null;
        if(row && row.length){
          row.map(item=>this.selectDataObj[item.id]=item)
//          console.log('选中')
        }else {
          delete(this.selectDataObj[index.id])
//          console.log('取消选中')
        }
//        console.log(this.selectDataObj)
      ...
      handleSelectionChange(val) {
        if(val.length){ // 全选
          val.map(item=>this.selectDataObj[item.id]=item)
//          console.log('全选')
        }else{
          if(this.selectTableTeml && this.selectTableTeml.length){
            this.selectTableTeml.map(item=>delete(this.selectDataObj[item.id]))
          }
          this.selectTableTeml = null;
//          console.log('取消全选')
        }
        ...
        if(res.data.length){
            res.data.map(item => {
              if(item.deviceList && item.deviceList.length){
                item.deviceList.map(cItem=>cItem.deviceTypeName = item.deviceTypeName)
              }
            });
        }
//        this.$nextTick(() => {
...
//        if (this.isSelected(this.selectOne)) {
          this.$emit('select', this.selectDataObj)
//        }
      },
    },
    components: {
