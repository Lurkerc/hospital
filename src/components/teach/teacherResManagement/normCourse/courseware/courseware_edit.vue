    ...
    <el-form class="editForm" inline="" ref="item" v-for="(item,index) in planDtoList" :model="item" :rules="rules" label-width="100px"
      ...
      <fieldset class="nPlanItem">
        ...
        <el-table ref="singleTable" :data="item.wareDtoListTemp.before">
          ...
          <el-table-column property="date" label="操作" width="120" align="center">
            <template scope="scope">
              <el-button type="warning" size="mini" @click="showFile(scope.row)">预览</el-button>
              <el-button type="danger" size="mini" @click="removeFile(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        ...
        <el-table ref="singleTable" :data="item.wareDtoListTemp.in_progress">
          ...
          <el-table-column property="name" label="显示名称">
          </el-table-column>
          <el-table-column property="address" label="文件名称">
          </el-table-column>
          <el-table-column property="address" label="文件格式">
          </el-table-column>
          <el-table-column property="address" label="文件大小">
          </el-table-column>
          <el-table-column property="address" label="转化状态">
          </el-table-column>
        </el-table>
        <el-col>
        ...
        <el-table ref="singleTable" :data="item.wareDtoListTemp.after">
          ...
          <el-table-column property="date" label="操作" width="120" align="center">
            <template scope="scope">
              <el-button type="warning" size="mini" @click="showFile(scope.row)">预览</el-button>
              <el-button type="danger" size="mini" @click="removeFile('after',index,scope)">删除</el-button>
            </template>
          </el-table-column>
          <el-table-column property="name" label="显示名称">
          </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 删除
      removeFile(type, wareIndex, delIndex) {
        console.log(type, wareIndex, delIndex)
      },
      /******************************************** 数据提交 ***************************************/
      // 保存
      saveToStore() {
