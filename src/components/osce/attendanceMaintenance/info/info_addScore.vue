<template>
  ...
  <div class="exmUserContent">
    ...
    <div class="add-score-head"><span class="add-score-l">{{examUserData.sceneName}}</span> <span class="add-score-r">满分({{ totalScore }})</span></div>
    <hr>
    <!-- 老师信息 -->
    ...
    <div class="exmContentMain " style="top: 31px;">
      <el-form :model="contentDataList" :rules="rules.addScore" ref="formValidate" label-width="80px">
    ...
    methods: {
      ...
      conductListData(data) {
        ...
        for (let i = 0; i < length; i++) {
          ...
          detailsList[i].detailsId = detailsList[i].id; // 评分表id
          detailsList[i].mark = '';
