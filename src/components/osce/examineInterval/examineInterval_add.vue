<template>
  ...
  <div style="height:100%;" id="stationMain" ref="stationMain">
    <!-- 基本信息 -->
    <div class="editForm">
      <el-form :model="info" :rules="rules" ref="info" label-width="84px">
        <el-row class="stationInfo">
          ...
          <el-col :span="6" :offset="1">
            <el-form-item label="开始时间：" prop="startTime">
              <el-date-picker v-model="info.startTime" :editable="false" format="yyyy-MM-dd HH:mm:ss" type="datetime" placeholder="选择考核开始时间">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="1" v-if="addType !== 'STANDARD'">
            <el-form-item label="结束时间：" prop="endTime">
              <el-date-picker v-model="info.endTime" :editable="false" format="yyyy-MM-dd HH:mm:ss" type="datetime" placeholder="选择考核结束时间">
              </el-date-picker>
            </el-form-item>
          </el-col>
        ...
        <div class="el-form selectUserBox">
          <fieldset style="height:90px;">
            <legend style="font-size:16px">&nbsp;&nbsp;考场管理员
              <el-button type="primary" size="mini" @click="addManager" icon="plus"></el-button>&nbsp;&nbsp;</legend>
            <div style="height:40px;overflow:auto;" v-if="managerList.length">
              <el-tag v-for="(item,index) in managerList" :key="item.managerId" :closable="true" type="success" @close="handleMClose(index)">
                {{item.userName}}
              </el-tag>
            </div>
            <p v-else>请选择考场管理员</p>
        ...
        <div class="el-form selectUserBox" style="margin-top:20px;">
          <fieldset style="height:128px;">
            <legend style="font-size:16px">&nbsp;&nbsp;参考人员
              <el-button type="primary" size="mini" @click="addUser" icon="plus"></el-button>&nbsp;&nbsp;</legend>
            <template v-if="userList.length">
              <div :style="{height:(addType === 'STANDARD' ? 40 : 80) + 'px',overflow:'auto'}">
                <el-tag v-for="(item,index) in userList" :key="item.userId" :closable="true" type="success" @close="handleClose(index)">
                  {{item.userName}}
                </el-tag>
              </div>
        ...
        <div class="el-form selectUserBox">
          <fieldset style="height:238px;">
            <legend style="font-size:16px">&nbsp;&nbsp;SP设置
              <el-button type="primary" size="mini" @click="addSP" icon="plus"></el-button>&nbsp;&nbsp;</legend>
              ...
              <el-collapse accordion>
                <el-collapse-item v-for="(item,index) in spList" :key="item.managerId">
                  ...
                  <el-checkbox-group v-model="item.scriptIds" v-if="!hasScriptList()">
                    <el-checkbox v-for="cItem in scriptList" :label="cItem.scriptId" :key="cItem.scriptId">{{ cItem.scriptName }}</el-checkbox>
                  </el-checkbox-group>
                  <p v-else>暂无病例</p>
                </el-collapse-item>
              </el-collapse>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /************************* 底部 *********************************************/
      setRoomListHeight() {
        let contenHeight = this.$refs.stationMain.offsetHeight;
        let roomHeight = this.$refs.stationRoomBox.offsetTop;
        this.stationRoomStyle.height = (contenHeight - roomHeight) + 'px';
      },
    },
    components: {
      room,
