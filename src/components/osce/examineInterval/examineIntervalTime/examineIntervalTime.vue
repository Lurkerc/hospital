    ...
    <div class="exContent">
      ...
      <div class="exRightBody">
        ...
        <template v-else>
          ...
          <div id="content" ref="content" style="position: absolute;left: 10px;right: 0;top: 45px;bottom: 10px;overflow: auto;">
            <student-time v-if="headerList[0]" :sceneType="sceneType" v-show="active === 'student'" :headerList="headerList" :tableList="tableList"></student-time>
            <teacher-time v-if="active === 'teacher'" :tableList="teacherTableList"></teacher-time>
            <student-time v-if="spHeaderList[0]" v-show="active === 'sp'" :headerList="spHeaderList" :tableList="spTableList"></student-time>
      ...
      <Modal
        ...
        :width="500">
        <modal-header slot="header"  :content="button[active]"></modal-header>
        <derive v-if="deriveModal" :type="active" :url="deriveData.url" @cancel="deriveModal = false"></derive>
        <div slot="footer"></div>
      </Modal>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      getExcel() {
        ...
        this.deriveData = {
          url:'/api/'+api[viewApiKeys[this.active]].path+'?id='+this.id,
          type:this.active
        };
