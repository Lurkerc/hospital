<template>
  <div class="editForm">
        ...
        <el-col :span="20" :offset="2">
          <el-form-item label="剧本：">
            <template v-for="(item,index) in formValidate.scriptList">
              <div class="scriptListItemText" v-if="item.id">
    ...
    <Modal :mask-closable="false" v-model="scoreShowModal" height="200" class-name="vertical-center-modal" :width="840">
      ...
      <score-show v-if="scoreShowModal" @cancel="cancel" :url-params="scoreApi.get" :id="childrenTodoId.id"></score-show>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 剧本删除（del） -->
...
<script>
  ...
  import scoreApi from '../assessmentContentScore/api'; // 评分表api
  ...
  import {
    assessmentContentCase as rules
  } from '../../rules'; // 表单验证
