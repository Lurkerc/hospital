<template>
  ...
  <div>
    ...
    <Modal :mask-closable="false" v-model="editModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="1000">
      ...
      <edit v-if="editModal" @cancel="cancel" @edit="subCallback" :operaility-data="operailityData" :url-params="api.modify" :get-url-params="api.get"
        :todoId="todoId"></edit>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 查看（view） -->
    <Modal :mask-closable="false" v-model="showModal" height="200" title="对话框标题" class-name="vertical-center-modal" :loading="true"
      ...
      <show v-if="showModal" @cancel="cancel" :operaility-data="operailityData" :id="todoId.id" :url-params="api.get"></show>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 删除（del） -->
    ...
    <!-- 模态框 导入评分表（score） -->
    <Modal :mask-closable="false" v-model="scoreModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="840">
      <modal-header slot="header" :content="button.scoreId"></modal-header>
      <score v-if="scoreModal" :id="treeId" :url="api" :treeRoad="treeRoad" @cancel="cancel" @score="subCallback" :operaility-data="operailityData" :url-params="api.save"></score>
      <div slot="footer"></div>
    </Modal>
    <!--导出到excel弹窗-->
    <Modal
    ...
    <!-- 模态框 复制（copy） -->
    <Modal :mask-closable="false" v-model="copyModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="840">
      <modal-header slot="header" :content="button.copyId"></modal-header>
      <copy v-if="copyModal" @cancel="cancel" @copy="subCallback" :operaility-data="operailityData" :url-params="api.add" :get-url-params="api.get"
        :todoId="todoId"></copy>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 移动（move） -->
...
<script>
  ...
  import add from './assessmentContentScore_add'; // 增加
  import edit from './assessmentContentScore_edit'; // 编辑
