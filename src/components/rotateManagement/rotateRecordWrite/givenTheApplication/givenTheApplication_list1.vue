...
<template>
  <div ref="givenTheApplication">
    <student v-if="userView === 'student'"></student>
...
<script>
  /*当前组件必要引入*/
  import student from './user_student/list'; // 学生（实习生、研究生、进修生、住院医）
  ...
  export default {
    ...
    methods: {
      //初始化请求列表数据
      init() {
        let view = ['student', 'teacher', 'kezr', 'dep']; // 视图
        let identify = [ // 角色
          ['SXS', 'YJS', 'JXS', 'ZYY'], // 实习生、研究生、进修生、住院医 （出科 | 查看）
          ['DJLS'], // 带教老师（出科 | 查看）
          ['KEZR'], // 科室主任（出科 | 查看）
