<template>
  <div>
    <div class="add-scope  to-channel">
      <el-table v-if="body[0]" :data="body" border @cell-click="cellClick" :max-height="500" :row-class-name="tableRowClassName"
        style="width: 100%">
...
<script>
  export default {
     ...
     * height :  最小高度
     * */
    props: ['format', 'height', 'url', 'data', 'extra', 'id', 'date', 'deptId', 'treeRoad'],
    data() {
      return {
        //保存按钮基本信息
        loadBtn: {
    ...
    methods: {
      ...
      confirm(isLoadingFun) {
        ...
        let myPromise = that.$util.queryData({
          ...
          url: (this.url),
          method: 'post',
          data: this.body == 0 ? this.tempArr : this.body
        })();
