        ...
        <el-row :gutter="10" class="table-back-one">
          ...
          <el-col :span="12">
            <el-form-item label="专业名称：" prop="majorCode">
              <el-select v-model="formValidate.majorCode" placeholder="请选择">
                <el-option v-for="(item,index) in majorOptions" :key="index" :label="item.value" :value="item.code"></el-option>
              </el-select>
    ...
    methods: {
      ...
      getDictionaryForMajor() {
        let opt = {
          ...
          ajaxParams: {
            url: pubApi.getDictionary + 'YJS_ZYDM',
            method: 'get'
          }
        }
        this.ajax(opt)
