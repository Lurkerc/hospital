...
export default {
  ...
  data() {
    ...
    if (!this.messTitle) {
      ...
      */
      messTitle = todoType[this.type] && {
            title: `确定要${todoType[this.type]}吗？`,
            access: `${todoType[this.type]}成功`,
            error: `${todoType[this.type]}失败`
          }
    } else {
    ...
    operateCallback(isLoadingFun) {
      ...
      myPromise.then(function (res) {
        ...
        if (_.isObject(responseData["status"]) && responseData["status"]["code"] == 0) {
          ...
          that.errorMess(that.operateMess.error);
          isLoadingFun(false);
        }
