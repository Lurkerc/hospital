...
<template>
  <div id="content" ref="content" class="modal">
    <el-form :model="formValidate" :rules="rules" ref="formValidate" inline label-width="100px">
      <el-row style="margin-bottom:0">
        ...
        <el-col :span="15" >
          ...
          <el-button type="primary" :disabled="disabled.enroll" @click="enroll">录取</el-button>
          <el-button type="warning" :disabled="disabled.enroll" @click="noEnroll">不拟录取</el-button>
          <el-button type="primary" >从Excel导入录取人员</el-button>
          <!--<el-button type="primary" @click="toChannel">从Excel导入录取人员</el-button>-->
          <el-button type="primary" >发送短信给录取人员</el-button>
          <el-button type="primary" >导出录取信息到Excel</el-button>
      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            label="短信通知">
            <template scope="scope">
              <el-button v-if="scope.row.messageStrtus=='NOTICE'">已通知</el-button>
          ...
          <el-table-column
            ...
            label="录取状态">
            <template scope="scope">
              {{scope.row.enterStrtus  |  typeText}}
            </template>
          </el-table-column>
        </el-table>
      </div>
    ...
    <Modal
      ...
      <operate v-if="enrollModal" type="enroll"  :operateData="enrollData" @operate="subCallback" @cancel="cancel" :operaility-data="operailityData"></operate>
      <div slot="footer"></div>
    </Modal>
    <!---->
    <!--不录取弹窗-->
    <Modal
      close-on-click-modal="false"
      height="200"
      v-model="noEnrollModal"
      title="对话框标题"
      class-name="vertical-center-modal"
      :loading="loading"
      :width="500">
      <modal-header slot="header" :content="noEnrollId"></modal-header>
      <operate v-if="noEnrollModal" type="noEnroll"  :operateData="noEnrollData" @operate="subCallback" @cancel="cancel" :operaility-data="operailityData"></operate>
      <div slot="footer"></div>
    </Modal>
    <!---->
    <!---->
    ...
    <Modal
      ...
      <toChannel v-if="toChannelModal" :url="url.excelImportEnroll.path"  @ranking="subCallback" @cancel="cancel" :operaility-data="operailityData"></toChannel>
      <div slot="footer"></div>
    </Modal>
    <!---->
    <!--导出弹窗-->
    <Modal
      ...
      :width="500">
      <modal-header slot="header" :content="toChannelId"></modal-header>
      <derive v-if="deriveModal" :url="url.excelExportEnterPerson.path" type="derive" messTitle="导出录取信息到Excel" @cancel="cancel" :operaility-data="operailityData"></derive>
      <div slot="footer"></div>
    </Modal>
    <!---->
  </div>
  ...
  export default{
    data() {
      return {
        ...
        enrollId:{id:'enrollId',title:'录取'},
        noEnrollId:{id:'noEnrollId',title:'不录取'},
        toChannelId:{id:'toChannelId',title:'从Excel导入录取人员'},
