      ...
      <el-table align="center" :height="dynamicHt" :context="self" :data="tableData" tooltip-effect="dark"
        ...
        <el-table-column label="操作" align="center" width="230">
          <template scope="scope">
            <!-- 汇报草稿|驳回状态才可修改 -->
            <el-button size="small" type="warning" @click="edit(scope.row)" v-if="['4','5','6','7','8'].indexOf(scope.row.status) > -1" :disabled="['5','6','8'].indexOf(scope.row.status) > -1">修改</el-button>
            <!-- 申请审核通过才可添加 -->
            <el-button size="small" type="warning" @click="add(scope.row)" v-else :disabled="scope.row.status != 3">添加</el-button>
            <!-- 填写之后即可查看 -->
            <el-button size="small" type="success" @click="show(scope.row)" :disabled="(+scope.row.status) < 4">查看</el-button>
            <!-- 汇报草稿|驳回状态才可上报 -->
            <el-button size="small" type="danger" @click="reported(scope.row)" :disabled="['4','7'].indexOf(scope.row.status) === -1">上报</el-button>
          </template>
        </el-table-column>
