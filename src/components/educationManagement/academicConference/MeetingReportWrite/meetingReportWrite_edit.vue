            ...
            <el-row v-if="formValidate.conferenceUserList.length">
              <el-col v-for="(item,index) in formValidate.conferenceUserList" :key="index">
                ...
                <template v-if="item.member == 'Y'">
                  <span>；请上传附件：</span>
                  <div>
                    <upload-file :uploadUrl="uploadUrl" @setUploadFiles="expenseFileEvent" :callData="{index}"
                                 :uploadFiles="item.fileArr" :uploadData="{mini:false}"></upload-file>
                  </div>
                </template>
              </el-col>
            </el-row>
        ...
        <date-group :dateGroup="{text:'',startDate:formValidate.startTime,endDate:formValidate.endTime}">
          <el-col :span="12" name="start">
            <el-form-item label="会议开始时间：" prop="startTime">
              <el-date-picker v-model="formValidate.startTime" type="datetime" :editable="false" placeholder="选择日期时间"
          ...
          <el-col :span="12" name="end">
            <el-form-item label="会议结束时间：" prop="endTime">
              <el-date-picker v-model="formValidate.endTime" type="datetime" :editable="false" placeholder="选择日期时间"
